...
<div id="smallNav" class="container-fluid d-block d-lg-none sticky-top bg-dark py-2">
    ...
    <div class="row mt-3">
        ...
        <div class="col-3 text-center">
            <a href="{{ route('profile-show', auth()->user()) }}"><img src="{{ auth()->user()->image_url }}" class="img-fluid rounded-circle img-xs"></a>
        </div>
    </div>
</div>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark sticky-top d-none d-lg-block" id="mainNav">
    <div class="container my-1">
        <a class="navbar-brand" href="{{ route('home') }}">{{ config('app.name', 'Ubuntu Social') }}</a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item">
                    <a href="{{ route('profile-show', auth()->user()) }}" class="nav-link pr-md-0"><img src="{{ auth()->user()->image_url }}" class="img-fluid rounded-circle img-xs mr-2">{{ auth()->user()->name }}</a>
                </li>
